...
/***************
 * @file sgemv.hpp
 * @brief 单精度矩阵-向量乘法（SGEMV）AVX2内核
 * @author SnifferCaptain
 * @date 2026-03-10
 *
 * 功能:
 * - 行向量 × 矩阵: C[1×n] = A[1×k] @ B[k×n]  (gemv_row_simd)
 * - 矩阵 × 列向量: C[m×1] = A[m×k] @ B[k×1]  (gemv_col_simd)
 *
 * 注意:
 * - 点积 m==1,n==1 请使用 sdot.hpp::sdot()
 * - 外积 k==1     请使用 sger.hpp::sger()
 *
 * NCOLS 模板参数控制列方向展开宽度（默认=4），可设为1..8的任意值
 ***************/
...
// ============================================================================
// 点积辅助：NCOLS列同时计算
// 利用 integer_sequence 展开，编译期确定 NCOLS，避免运行时分支
// ============================================================================

namespace detail {

/// @brief 初始化 NCOLS 个累加器
...
/// @brief 对 NCOLS 列执行一步 FMA（每行A一次加载，B列各自加载）
template<int NCOLS, int... Is>
...
/// @brief 同时计算 NCOLS 个列的点积（A行向量与每列B的内积）
/// @tparam NCOLS 展开的列数（任意正整数）
/// @param A     行向量 A[k]（连续，步幅=1）
/// @param Bcols B的 NCOLS 个列的指针数组（每列各自连续）
/// @param k     共享的内维长度
/// @param sums  输出 sums[NCOLS]
template<int NCOLS>
