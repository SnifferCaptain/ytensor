...
- **默认 header-only**（标准形态）
- **single-header 打包**（`ytensor_single.hpp`）
- **`YT_USE_LIB` 加速后端**（API 不变，仅多一步链接库）

默认模式仍然是完整 header-only。`YT_USE_LIB` 仅改变实现归属（将封闭 runtime 核心预编译），不会改变公开 API 与自定义类型能力。
...
1. **下载单头文件**
   ...
   从正式发布包获取生成的 `ytensor_single.hpp`。开发源码树只保留 `single-header/packer.py`，不保存生成产物。
...
### 方法三：使用 `YT_USE_LIB`（编译加速）

当你希望减少用户工程里的重复编译时，可以使用该模式。
...
2. **定义 `YT_USE_LIB` 并链接 `libytensor`**
...
:::info
`YT_USE_LIB` 的**唯一额外要求**是链接预编译库；API、返回类型、语义、支持类型集合和自定义类型工作流保持等价。
:::
