...
| `YT_USE_YBLAS` | 跟随 `YT_USE_AVX2` | 是否启用 YBLAS 后端 |
| `YT_USE_AVX2` | 自动检测 | 是否启用 AVX2+FMA 物理内核 |
| `YT_PREINSTANTIATE_TEMPLATES` | `1` | 是否预实例化常用类型模板 |
| `YT_USE_LIB` | `0` | 使用预编译 runtime 后端（需链接 `libytensor`） |
...
- **启用时**：`yt::info::MatmulBackend::Eigen` 后端可用，矩阵乘法性能显著提升
- **禁用时**：不影响 `Naive` 和当前构建中已启用的 YBLAS 后端
...
控制是否编译 AVX2+FMA 物理内核。YBLAS 当前使用这组内核加速矩阵乘法和卷积等运算。
...
推荐通过编译选项启用 AVX2 和 FMA，并让 YTensor 自动检测。若编译目标没有同时启用这两个特性，
手动定义 `YT_USE_AVX2=1` 会在预处理阶段被拒绝。`YT_USE_YBLAS` 默认跟随
`YT_USE_AVX2`；如果只想禁用 YBLAS 后端，可显式定义 `YT_USE_YBLAS=0`。
...
3. 宏不能替代编译器 target 选项，不应使用它绕过 feature detection
:::

### 影响

- **启用时**：YBLAS 默认可用，`yt::info::MatmulBackend::YBLAS` 可选择自研 SIMD 实现
- **禁用时**：仅能使用 `Naive` 和 `Eigen` 后端

实际性能取决于矩阵 shape、storage layout、线程数、编译器和 CPU。应使用与实际负载一致的 benchmark，而不是假定相对 Naive 或 Eigen 的固定加速倍数。

底层 Level-1/2/3 的启用方式、接口选择、stride 和线程配置见 [BLAS 总览](../api/blas/overview.mdx)。
...
:::info
在 v0.14 中，此选项主要在构建或使用 `YT_USE_LIB` 预编译库后端时更有意义。
:::
...
将 YTensor 切换到预编译后端，用于承载封闭 runtime 核心（`YTensorBase` 的 runtime 分发 / copy-cast-clone / I/O 相关实现），同时保持开放模板逻辑仍在头文件。

### 默认行为

默认关闭（`YT_USE_LIB = 0`）。关闭时保持完整 header-only 行为。

### 设置方式

可在包含头文件前定义宏，或通过编译/CMake 参数设置：
...
- **唯一差异**：额外链接 `libytensor`。
- 公开 API、返回类型、支持类型集合与自定义类型工作流与 header-only/single-header 保持等价。
...
int main() {
    ...
    #if YT_USE_YBLAS
        std::cout << "  YBLAS 后端: 已启用" << std::endl;
    #else
        std::cout << "  YBLAS 后端: 未启用" << std::endl;
    #endif
...
### Q: Eigen 和 YBLAS 可以同时启用吗？

**A**: 可以。YTensor 会优先使用 YBLAS 后端，但您仍然可以手动指定 Eigen 后端。YBLAS
当前的物理实现要求 AVX2+FMA 编译目标。
