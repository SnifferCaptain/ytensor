...
`zeros()` 使用 `fill()` 方法填充元素。如果只需要分配内存而不关心初始值，直接使用构造函数会更快。
:::
...
在多线程环境下，`randn` 使用互斥锁（`yt::info::rngMutex`）保护随机数生成器，可能成为性能瓶颈。
...
`seed()` 设置的是全局共享的随机数生成器 `yt::info::gen`，因此：
...
`seed()` 内部使用互斥锁 `yt::info::rngMutex` 保护，是线程安全的。
