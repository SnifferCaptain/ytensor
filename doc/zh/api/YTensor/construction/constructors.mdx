...
创建一个默认张量，所有维度大小初始化为 1 (例如 `[1, 1, ..., 1]`)，以增强代码鲁棒性。不分配数据内存。

### 返回值

- 返回一个形状为 `[1, 1, ..., 1]`（dim个1）的张量
...
void example() {
    ...
    // 查看形状
    std::cout << empty.shape()[0] << std::endl;  // 输出: 1
    std::cout << empty.size() << std::endl;      // 输出: 1
...
:::warning
默认张量虽然形状为 1，但可能未分配数据内存（或仅分配占位符）。请在使用前确保正确分配空间 (`reserve` 或赋值)。
:::
...
| 构造方式 | 示例 | 内存分配 | 适用场景 |
| --- | --- | --- | --- |
| 默认构造 | `YTensor<float, 2>()` | 否 | 延迟初始化 (维度置1) |
...
int main() {
    ...
    std::cout << "Empty size: " << empty.size() << std::endl;  // 1
