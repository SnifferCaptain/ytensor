...
```cpp
static constexpr int ndim = dim; // 编译期rank常量
int shapeSize() const;           // runtime metadata rank
```

### 核心功能描述

`YTensor<T, dim>::ndim` 是编译期rank常量，`shapeSize()` 返回runtime活跃layout rank。
...
```cpp
auto a = YTensor<float, 3>(2, 3, 4);
int dims1 = decltype(a)::ndim; // 3（编译期）
...
| 方法 | 来源 | 特点 |
|------|------|------|
| `YTensor<T, dim>::ndim` | `YTensor<T, dim>` | 编译期常量 |
| `shapeSize()` | `YTensorBase` | runtime活跃layout rank |
