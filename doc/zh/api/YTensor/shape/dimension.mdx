...
| 方法 | 功能 | 维度变化 | 说明 |
| --- | --- | --- | --- |
| `unsqueeze()` | 插入大小为 1 的维度 | +1 | 返回runtime `YTensorBase`视图 |
| `squeeze()` | 移除大小为 1 的维度 | -1 或 -N | 返回runtime `YTensorBase`视图 |
| `repeat()` / `repeat_()` | 沿维度重复 | 不变 | 需要拷贝 |
| `unfold()` | 滑动窗口展开 | +1 | 返回runtime `YTensorBase`视图 |
...
固定rank的 `YTensor<T, dim>` 会隐藏改变rank的原地方法。请使用返回的 `YTensorBase` 视图，或通过检查转换构造 `YTensor<T, newDim>`。
