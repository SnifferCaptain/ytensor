...
通过多维坐标访问张量元素。这是**推荐的访问方式**，性能优于 `operator[]`。支持负索引（循环索引）。
...
// ❌ 避免：使用 at() 尝试负索引 (如果类型不支持)
// tensor.at(-1, -1, -1);  // YTensor/YTensorBase的 at() 都支持负索引, 这是安全的
```
