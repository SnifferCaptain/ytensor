...
当使用 `Func` 版本并走 AVX2 后端时，若谓词对象额外提供以下方法，可进行块级剪枝：
...
// 使用 AVX2 后端
auto c3 = a.matmul(b, yt::info::MatmulBackend::AVX2);
```
...
矩阵乘法的性能与矩阵的内存布局（主序）密切相关。不同后端对主序的敏感度不同：

| 后端 | 左矩阵主序 | 右矩阵主序 | 性能说明 |
|------|----------|----------|----------|
| **Naive** | 行主序 | 列主序 | 此内存布局下稳定达到 ~20 GFlops |
| **Naive** | 其他 | 其他 | 内存访问模式不佳，性能下降 |
| **Eigen** | 行主序 | 行主序 | 最佳性能（~75 GFlops） |
| **Eigen** | 其他 | 其他 | 性能显著下降（内存排布不利于计算） |
| **AVX2** | 任意 | 任意 | **主序不敏感**，稳定高性能（~95 GFlops） |

:::tip
**关于主序的建议**

- 如果使用 **Eigen** 后端，确保两个矩阵都是行主序（连续内存），否则性能可能远不如预期
- **Naive** 后端在左矩阵行主序、右矩阵列主序时性能最佳，在这种情况下甚至可能比 Eigen 主序不合适的情况更快
- **AVX2** 后端完全不挑主序，始终能提供稳定的高性能
:::
