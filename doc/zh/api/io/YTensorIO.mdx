...
```cpp
auto t = yt::YTensor<float, 2>::ones(3, 3);
yt::io::YTensorIO io;
...
:::info
**非 POD 类型支持**
从 v0.1 版本开始，YTensor IO 支持非 POD 类型（如 `std::string`）的存储。
这需要该类型在注册时提供了 `serialize` 和 `deserialize` 回调函数。详情请参考 [类型注册](../infos/types.mdx)。

**注意**：由于当前实现限制，`serialize` 生成的数据大小不应显著大于对象的内存大小（`sizeof(T)`），否则在读取时可能会因缓冲区预分配不足而导致失败。建议仅存储短字符串或小型结构体。
:::
