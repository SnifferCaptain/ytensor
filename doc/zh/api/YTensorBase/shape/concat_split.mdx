...
`concat` 支持所有已注册的数据类型，包括：
- **POD 类型** (如 `float`, `int`): 使用高效的 `memcpy`。
- **非 POD 类型** (如 `std::string`): 使用注册的 `copyConstruct` 进行深拷贝。

:::note
对于非 POD 类型，必须确保该类型已通过 `yt::types::registerType` 正确注册，并提供了拷贝构造函数。
:::
