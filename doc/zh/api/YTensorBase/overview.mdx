...
YTensorBase 是 YTensor 库的**运行时张量门面**，也是 `YTensor<T, dim>` 的非模板基类。它提供类型擦除接口，同时保持 layout 分发边界清晰可见。
...
应用代码
    |
    +-- YTensor<T, dim> -------- 编译期 dtype/rank 检查与 typed 热路径
    |          |
    |          +-------------------+
    |                              |
    +-- YTensorBase ---------------+-- runtime facade
       ...
       offset, shape, stride         offsets, values, shape
                   |                               |
                   v                               +-- 仅 metadata/lifetime
             yt::strided                               groundwork
    +-----------+--------+--------+--------+
    |           |        |        |        |
...
横切命名空间：yt::utils、yt::info、yt::io、yt::function
```

`YTensorBase` 负责运行时选择、校验和共享张量状态，不持有 dense 算法实现。Strided 操作会路由到 `yt::strided` 的五个 owner 之一：`broadcast`、`view`、`copy`、`reduce` 或 `matmul`。运行时数值操作在进入元素循环前选择一次 `yt::type::YDTypeKernels` 条目；当编译期 dtype 和 rank 有价值时，`YTensor<T, dim>` 保留直接 typed 路径。矩阵运算可以继续委托给 `yt::blas` backend。

`YLayout` 是由 `YLayoutType` 选择 active metadata 的 tagged union。Strided metadata 是 dense `offset`、`shape` 和 `stride` 的唯一权威来源。Nested 目前只提供 metadata 和对象生命周期基础，尚未实现完整 Nested 算子；当前架构也不包含 Sparse layout 支持。
...
| 类型检查 | **编译时** | 运行时 |
| 维度检查 | **编译时** | 运行时 |
| 性能 | 直接 typed 热路径 | 运行时检查 layout 与 dtype |
| 灵活性 | 类型固定 | **类型动态** |
| 存储 | 不同类型不兼容 | **统一接口** |
| 安全性 | **编译期保证** | 运行期检查 |
...
// 2. 拷贝构造
YTensorBase b = a;  // 共享 values storage，独立复制 metadata
...
// dtype或rank不匹配时抛出std::invalid_argument
...
```cpp
YTensorBase a({2, 3, 4}, "float32");
YTensorBase b = a;  // values 浅拷贝（共享 storage）

b.at<float>(0, 0, 0) = 1.0f;
// a.at<float>(0, 0, 0) == 1.0f

auto c = a.slice(0, 0, 1);  // metadata 独立、storage 共享的 view
c.at<float>(0, 0, 0) = 1.0f;
// a.at<float>(0, 0, 0) == 1.0f

auto d = a.clone();  // 独立 storage
...
```cpp
YTensorBase a({2, 3, 4}, "float32");
YTensorBase b = a;  // 增加共享 storage 的所有权计数

// 两个句柄引用相同的 values storage
b.at<float>(0, 0, 0) = 1.0f;
// a.at<float>(0, 0, 0) == 1.0f
...
// dtype错误时抛出std::invalid_argument
...
运行时数学操作不会为每个元素执行虚函数调用或 dtype registry 查找。`YTensorBase` 先选择 active layout，Strided owner 完成操作校验，再在元素循环前选择 `yt::type::YDTypeKernels` 回调。当编译期信息可以避免运行时 dtype 检查时，`YTensor<T, dim>` 保留直接 typed 访问路径。

```cpp
// typed facade：dtype 和 rank 是编译期属性
YTensor<float, 3> a({100, 100, 100});
for (int i = 0; i < 100; i++) {
    a.at(i, i, i) = 1.0f;
}

// runtime facade：运行时校验请求的访问类型
...
实际成本取决于具体操作、dtype、layout 和所选 backend。应以应用级实测为准，而不是采用固定的开销估算。
