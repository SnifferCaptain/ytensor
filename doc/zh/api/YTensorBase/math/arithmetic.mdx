...
- `bfloat16`, `float16`, `float8_e5m2`, `float8_e4m3`, `float8_e8m0`, `float8_ue8m0`
- `%` 运算符在浮点类型下使用 `fmod` 实现
...
- builtin runtime kernel支持上述数值类型和 `bool`；custom dtype需要注册comparison kernel
...
// 非原地版本
YTensorBase operator+(const YTensorBase& other) const {
    auto opShape = yt::utils::computeBroadcastShape({this->shape(), other.shape()});
