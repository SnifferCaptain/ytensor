...
本文档介绍 `YTensorBase` 的 `sum()`、`mean()` 和 `max()`。
...
| `mean(axis)` | `YTensorBase` | 沿指定轴进行同dtype平均 |
| `mean(axes)` | `YTensorBase` | 沿规范化后的多个轴依次进行同dtype平均 |
...
`mean()` 保持rank和dtype。负轴会规范化，多轴会排序并去重；整数保持同dtype整数除法。归约空轴时返回零初始化结果。
...
| 情况 | 异常类型 |
| --- | --- |
| 0维张量调用 sum/mean/max | `std::runtime_error` |
| max归约空轴 | `std::domain_error` |

---

## 性能说明

- 使用 `yt::utils::parallelFor` 自动并行化
