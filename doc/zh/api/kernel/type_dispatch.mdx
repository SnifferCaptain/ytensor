# 类型分发 (Type Dispatch)

本文档介绍 `yt::type` 命名空间中的运行时类型分发机制。

## 概述

### 什么是类型分发？

在 YTensor 中，`YTensorBase` 使用字符串 `dtype`（如 `"float32"`）在运行时表示数据类型，而 `YTensor<T, dim>` 在编译时确定类型。**类型分发**的作用就是将运行时的 dtype 字符串映射到编译时的具体类型 `T`，从而调用正确的模板函数。

### 为什么需要类型分发？

假设你需要编写一个函数来处理 `YTensorBase`，但内部需要访问具体类型的数据：

```cpp
void processData(const YTensorBase& tensor) {
    // tensor.dtype() 可能是 "float32"、"int64" 等
    // 如何根据 dtype 调用正确的模板函数？
    
    // ❌ 传统方式：冗长的 if-else
    if (tensor.dtype() == "float32") {
        const float* data = tensor.data<float>();
        // ...
    } else if (tensor.dtype() == "int64") {
        const int64_t* data = tensor.data<int64_t>();
        // ...
    }
    // 需要为每种类型写一遍！
    
    // ✅ 使用类型分发：简洁高效
    yt::type::dispatch<yt::type::AllNumericTypes>(
        tensor.dtype(),
        [&]<typename T>() { // C++20 泛型 Lambda
            const T* data = tensor.data<T>();
            // T 会被自动推导为正确的类型
        },
        "processData"
    );
}
```

## 预定义类型列表

为了方便使用，YTensor 预定义了一些常用的类型列表：

| 类型列表 | 包含的类型 |
| --- | --- |
| `StandardFloatTypes` | `float`, `double` |
| `StandardIntTypes` | `int8`, `int16`, `int32`, `int64`, `uint8`, `uint16`, `uint32`, `uint64` |
| `IntegerTypes` | 同 `StandardIntTypes` |
| `StandardNumericTypes` | 标准浮点 + 标准整数 |
| `ExtendedFloatTypes` | `bfloat16`, `float16`, `float8_e5m2`, `float8_e4m3` 等 |
| `AllNumericTypes` | 所有上述类型 |

## `dispatch`

### 函数签名

```cpp
template<typename TypeListT, typename Func>
bool dispatch(const std::string& dtype, Func&& func);

template<typename TypeListT, typename Func>
void dispatch(const std::string& dtype, Func&& func, const std::string& opName = "dispatch");
```

### 核心功能描述

根据 `dtype` 字符串，在 `TypeListT` 中查找匹配的 C++ 类型 `T`，并调用泛型 lambda 函数 `func.operator()<T>()`。

*   `dispatch`: 如果找到匹配类型并成功执行，返回 `true`；否则返回 `false`。
*   `dispatch`: 如果未找到匹配类型，抛出 `std::runtime_error` 异常。

#### 嵌套类型自动解析

`dispatch` 支持**嵌套 dtype 的自动解析**。当传入的 dtype 字符串是嵌套格式（如 `"YTensorBase<float32>"`）时：

1. 首先尝试直接匹配完整的 dtype 字符串
2. 如果直接匹配失败，自动调用 `getBaseDtype()` 提取基础类型
3. 使用提取的基础类型（如 `"float32"`）进行二次匹配

这一特性使得 `matView()` 返回的张量可以被正确 dispatch：

```cpp
YTensorBase tensor({3, 4, 5}, "float32");
auto matView = tensor.matView();  // dtype: "YTensorBase<float32>"

// dispatch 会自动解析 "YTensorBase<float32>" -> "float32"
yt::type::dispatch<yt::type::AllNumericTypes>(matView.dtype(), [&]<typename T>() {
    std::cout << "Base type size: " << sizeof(T) << std::endl;  // 输出: 4
});
```

### 使用示例

假设我们需要编写一个打印函数，支持所有数值类型：

```cpp
#include "ytensor_kernel.hpp" // 或者是 type_dispatch.hpp

void printValue(const yt::YTensorBase& tensor) {
    // 定义一个泛型 lambda
    auto kernel = [&]<typename T>() {
        // 在这里，T 是具体的类型（如 float, int 等）
        // 我们可以安全地通过 data<T>() 访问数据
        const T* data = tensor.data<T>();
        std::cout << "Value: " << data[0] << std::endl;
    };

    // 分发！
    // 尝试在 AllNumericTypes 中匹配 tensor.dtype()
    yt::type::dispatch<yt::type::AllNumericTypes>(
        tensor.dtype(), 
        kernel, 
        "printValue"
    );
}
```

## Pairwise Cast

运行时 pairwise 转换使用 `yt::type::registerCastKernel`，避免实例化所有 source/destination 类型组合。核心 runtime math 使用 `YDTypeKernels`；`dispatch` 仅保留给 owner 内部的 typed fallback 路径。

---

## 最佳实践

1.  **使用泛型 Lambda (C++20)**：配合 `[&]<typename T>() { ... }` 语法，代码最简洁。
2.  **选择合适的 TypeList**：尽量只包含你真正支持的类型列表，以减少编译时间及二进制大小（虽然模板实例化是按需的，但错误的匹配尝试也可能有开销）。
3.  **错误处理**：对于必须成功的操作，优先使用 `dispatch`，它能提供清晰的错误信息。

---

## 相关内容

- [类型系统](../infos/types.mdx) - `yt::type` 命名空间与类型注册
- [YTensorBase](../YTensorBase/) - 运行时动态类型张量
