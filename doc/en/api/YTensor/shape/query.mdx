...
```cpp
static constexpr int ndim = dim; // Compile-time rank constant
int shapeSize() const;           // Runtime metadata rank
```

### Core Function Description

`YTensor<T, dim>::ndim` is the compile-time rank constant. `shapeSize()` checks the runtime layout rank.
...
```cpp
auto a = YTensor<float, 3>(2, 3, 4);
int dims1 = decltype(a)::ndim; // 3 (compile-time constant)
int dims2 = a.shapeSize();     // 3 (runtime metadata)
```
...
|------|------|------|
| `YTensor<T, dim>::ndim` | `YTensor<T, dim>` | Compile-time constant. |
| `shapeSize()` | `YTensorBase` | Runtime active-layout rank. |
