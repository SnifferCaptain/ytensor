...
YTensor<float, 2> b(base);  // Throws std::invalid_argument if dtype/rank mismatch
```

:::warning
Constructing `YTensor<T, dim>` from `YTensorBase` validates dtype, element size, and rank. A mismatch throws `std::invalid_argument`.
:::
