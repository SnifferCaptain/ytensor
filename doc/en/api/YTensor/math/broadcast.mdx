...
- **Custom Function Broadcasting**: The `yt::strided::broadcast()` function applies custom operations across tensors.
...
int main() {
    ...
    auto c = yt::strided::broadcast([](const float& x, const float& y) {
        return x + y;
    }, a, b);
...
auto c = yt::strided::broadcast([](const float& x, const float& y) -> double {
...
auto result = yt::strided::broadcast(
...
`broadcast` is automatically parallelized using `yt::utils::parallelFor`:
...
auto c = yt::strided::broadcast([](const float& x, const float& y) -> int {
...
auto result = yt::strided::broadcast([](const float& x, const float& y) {
    return x + y;
}, a, b);
```
