...
Computes a same-dtype sum followed by same-dtype division along the specified axes.
...
*   Integer tensors use integer division; multi-axis means apply this operation sequentially after axis normalization.
*   An empty reduced axis yields the zero-initialized result.
...
`mean()` accumulates in `T` and divides once by the axis length:
...
T mean = axisLen == 0 ? T(0) : sum / T(axisLen);
```
...
**Threshold**: `yt::info::minParOps`. Parallelism is enabled when the number of elements exceeds this threshold.
...
// mean() uses the same float accumulation as sum()
float mean = a.mean();
```
:::
