...
* **Type**: `void`
* **Memory Behavior**: The target tensor will share the source tensor's `YMemory` storage handle. Modifying data in either tensor will affect the other.
...
:::warning
**Shared Memory Risk**
`shallowCopyTo` causes two tensors to share the same underlying storage through `YMemory`. Any changes to the data of one tensor will be reflected in the other. If you need a completely independent copy, use `clone()` instead.
:::
...
inline YTensorBase YTensorBase::clone() const {
    ...
    auto typeInfoOpt = yt::type::getTypeInfo(_dtype);
...
inline YTensorBase& YTensorBase::copy_(const YTensorBase& src) {
    ...
    if (this->isContiguous() && src.isContiguous()) {
        std::memcpy(_memory.get() + _offset * elemSize, 
                    src._memory.get() + src._offset * elemSize, 
                    total * elemSize);
        return *this;
    }
