...
:::tip
`yt::type::getTypeRegistry()` returns a snapshot of custom registrations. Use `getBuiltinTypeInfo()` for builtin lifecycle metadata and `getTypeInfo()` to resolve either category.
:::
