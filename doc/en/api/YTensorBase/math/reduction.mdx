...
This document describes `sum()`, `mean()`, and `max()` for `YTensorBase`.
...
| `mean(axis)` | `YTensorBase` | Compute the same-dtype mean along one axis. |
| `mean(axes)` | `YTensorBase` | Compute sequential same-dtype means along normalized axes. |
...
`mean()` preserves rank and dtype. Negative axes are normalized; multi-axis inputs are sorted and deduplicated. Integer tensors use same-dtype integer division. Reducing an empty axis yields the zero-initialized result.
...
| Case | Exception Type |
| --- | --- |
| `ndim < 1` for `sum`/`mean`/`max` | `std::runtime_error` |
| Empty reduced axis for `max` | `std::domain_error` |

---

## Performance Notes

- These operations are automatically parallelized using `yt::utils::parallelFor`.
