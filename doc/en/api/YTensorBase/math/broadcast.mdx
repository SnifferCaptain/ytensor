...
```cpp
    ...
    yt::utils::parallelFor(0, totalSize, [&](int index) {
        auto indices = yt::strided::computeBroadcastIndices(...);
...
- **OpenMP Parallelization**: Automatically parallelized via `yt::utils::parallelFor`.
...
// ❌ Error: Shape {3} of 'a' cannot hold the broadcast result {3, 4}
