...
YTensor supports three equivalent usage modes:

- **Default header-only** (standard mode)
- **Single-header package** (`ytensor_single.hpp`)
- **`YT_USE_LIB` accelerated backend** (same API, plus one extra link step)

By default, YTensor remains header-only. `YT_USE_LIB` only changes implementation ownership (precompiled runtime core) and does not change public API or custom-type support.
...
### Method 1: Using a Release Single Header

This is the simplest way to integrate the library and is suitable for most users.

1. **Download the Single Header**

   Get the generated `ytensor_single.hpp` from a release package. The development source tree keeps only `single-header/packer.py`, not the generated artifact.
...
### Method 3: Using `YT_USE_LIB` (Compile-time Acceleration)

Use this mode when you want to reduce repeated compilation in consumer TUs.

1. **Build the precompiled backend library**

   ```bash
   cd /path/to/ytensor
   cmake -S . -B build-lib -DYTENSOR_BUILD_LIB=ON
   cmake --build build-lib -j8
   ```

2. **Enable `YT_USE_LIB` and link `libytensor`**
...
:::info
In `YT_USE_LIB` mode, the **only extra requirement** is linking the prebuilt library. API, return types, semantics, supported type set, and custom-type workflow stay equivalent.
:::
