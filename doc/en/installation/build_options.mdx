...
| `YT_USE_YBLAS` | Follows `YT_USE_AVX2` | Whether to enable the YBLAS backend |
| `YT_USE_AVX2` | Auto-detected | Whether to enable AVX2+FMA physical kernels |
| `YT_PREINSTANTIATE_TEMPLATES` | `1` | Whether to pre-instantiate common type templates |
| `YT_USE_LIB` | `0` | Use precompiled runtime backend (requires linking `libytensor`) |
...
- **When enabled**: The `yt::info::MatmulBackend::Eigen` backend becomes available, significantly improving GEMM performance.
- **When disabled**: `Naive` and any YBLAS backend enabled by the current build remain available.
...
Controls whether AVX2+FMA physical kernels are compiled. YBLAS currently uses these kernels for matrix
multiplication, convolution, and related operations.
...
The recommended setup is to enable AVX2 and FMA in the compiler target and let YTensor detect them.
Defining `YT_USE_AVX2=1` without both compiler features is rejected at preprocessing time.
`YT_USE_YBLAS` follows `YT_USE_AVX2` by default; define `YT_USE_YBLAS=0` to disable only the YBLAS backend.
...
3. The macro does not replace compiler target options; do not use it to bypass feature detection.
:::

### Impact

- **When enabled**: YBLAS is available by default, and `yt::info::MatmulBackend::YBLAS` selects the custom SIMD implementation.
- **When disabled**: Only the `Naive` and `Eigen` backends can be used.

Actual performance depends on matrix shape, storage layout, thread count, compiler, and CPU. Use a workload-specific benchmark instead of assuming a fixed speedup over Naive or Eigen.

See [BLAS Overview](../api/blas/overview.mdx) for Level-1/2/3 requirements, interface selection, strides, and thread configuration.
...
:::info
In v0.14, this option is primarily useful when building or consuming the `YT_USE_LIB` precompiled backend.
:::
...
Switches YTensor to the precompiled backend for the closed runtime core (`YTensorBase` runtime dispatch / copy-cast-clone / I/O related implementation), while keeping template/open-world logic in headers.

### Default Behavior

Disabled by default (`YT_USE_LIB = 0`). In this mode, YTensor behaves as full header-only.

### Setup

Define macro before including headers, or via compiler/CMake flags:
...
```bash
g++ -std=c++20 -DYT_USE_LIB main.cpp \
...
```cmake
target_compile_definitions(my_target PRIVATE YT_USE_LIB=1)
target_include_directories(my_target PRIVATE /path/to/ytensor /path/to/ytensor/lib/include)
target_link_directories(my_target PRIVATE /path/to/ytensor/lib/bin)
target_link_libraries(my_target PRIVATE ytensor ZLIB::ZLIB OpenMP::OpenMP_CXX)
```

### Impact

- **Only difference**: an extra link step to `libytensor`.
- Public API, return types, type support, and custom-type workflow remain equivalent to header-only/single-header modes.
...
#if YT_USE_YBLAS
    MatmulBackend::YBLAS    // Highest priority
#elif YT_USE_EIGEN
...
int main() {
    ...
    #if YT_USE_YBLAS
        std::cout << "  YBLAS Backend: Enabled" << std::endl;
    #else
        std::cout << "  YBLAS Backend: Disabled" << std::endl;
    #endif
    ...
    switch(yt::info::defaultMatmulBackend) {
...
### Q: Can Eigen and YBLAS be enabled simultaneously?

**A**: Yes. YTensor prefers YBLAS, but you can still request Eigen explicitly. The current YBLAS physical
implementation requires an AVX2+FMA compiler target.
