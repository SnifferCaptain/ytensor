...
#include "include/tokenlizer.hpp"
...
int main() {
    ...
    // 初始化模型配置
    qwen3::Qwen3Config config;
    config.init("../model");
    
    // 初始化并加载模型
    qwen3::Qwen3ForCausalLM model;
    model.init(config);
    
    std::cout << "loading model weights..." << std::endl;
    std::string weight_path = "../model/qwen3-0.6b.yt";// 【在这里填入模型权重文件路径】
    ...
    while (true) {
        ...
        // 参数为：输入token序列，最大上下文长度，结束符id，输出token的回调函数，温度
        model.generate(new_ids, 4096, eos_id, [&](int token_id) {
