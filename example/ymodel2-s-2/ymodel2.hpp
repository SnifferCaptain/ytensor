...
class KVCache {
public:
    YTensor<float, 4> buffer;  // [b, 2, l, hd] or [b, 2, hd, l]
    ...
    void append(const YTensor<float, 4>& new_kv);  // append new_kv [b, 2, l, hd] to buffer
    YTensor<float, 4> get() const;  // 获取当前KV缓存（可能乱序）
    bool empty() const { return cur_len == 0; }
    bool full() const { return cur_len >= max_len; }
    // 获取剩余可用空间（对于循环buffer，始终返回max_len - cur_len）
    int remaining() const { return max_len - cur_len; }
    // 获取当前全局位置（用于RoPE）
    int get_global_position() const { return total_written; }
    // 获取causal mask [query_len, kv_len]
    YTensor<float, 2> get_mask(int query_len) const;
};
...
struct RoPECache {
    YTensor<float, 2> cos, sin;
    void precompute(int dim, int max_len, float theta);
};

class RMSNorm {
public:
    YTensor<float, 1> weight;
    float eps = 1e-8f;
    YTensor<float, 3> forward(const YTensor<float, 3>& x) const;
    void forward_(YTensor<float, 3>& x) const;
};

class FFN {
public:
    YTensor<float, 2> up, down;
...
class PEGA2 {
public:
    YTensor<float, 2> qkv_0, qkv_1, o;
    int hidden_size, num_heads, head_dim;
    float rsqrt_dim;
    std::vector<int> qkv_list;
    
    // 传入KVCache指针，在内部更新cache，返回输出tensor
    YTensor<float, 3> forward(
        const YTensor<float, 3>& x,
        const YTensor<float, 2>& cos,
        const YTensor<float, 2>& sin,
...
public:
    ...
    YTensor<float, 2> forward(const YTensor<int, 2>& ids);
    // generate: 智能生成
    std::vector<int> generate(const std::vector<int>& new_ids, int max_tokens, int eos = 2,
                              std::function<void(int)> on_token = nullptr);
    void reset_kv_cache();  // 重置KV cache（新对话时调用）
    int get_kv_cache_len() const;  // 获取当前KV缓存的token长度
    int get_max_context_len() const;  // 获取最大上下文长度
};

namespace ops {
    template<int dim> YTensor<float, dim> linear(const YTensor<float, dim>& x, const YTensor<float, 2>& weight);
    
    template<int dim> YTensor<float, dim>& gelu_(YTensor<float, dim>& x);

    void rope(YTensor<float, 4>& q, YTensor<float, 4>& k, const YTensor<float, 2>& cos, const YTensor<float, 2>& sin);

    YTensor<float, 5> repeat_kv(const YTensor<float, 4>& x, int n);  // 零拷贝版本，返回5D张量
}
