...
inline void sgemm(
    ...
    if (a_rowmajor_exact && b_colmajor_exact && c_rowmajor_exact && alpha == 1.0f && beta == 0.0f) {
        static thread_local AlignedBuffer b_col_to_row;
        b_col_to_row.ensure((size_t)k * (size_t)n);
        float* B_row = b_col_to_row.data;
        for (int p = 0; p < k; ++p)
            for (int j = 0; j < n; ++j)
                B_row[p * n + j] = B[p * rsb + j * csb];
        sgemm_rowmajor(A, B_row, C, m, n, k);
        return;
    }

    if (a_colmajor_exact && b_rowmajor_exact && c_rowmajor_exact && alpha == 1.0f && beta == 0.0f) {
        static thread_local AlignedBuffer a_col_to_row;
        a_col_to_row.ensure((size_t)m * (size_t)k);
        float* A_row = a_col_to_row.data;
        for (int i = 0; i < m; ++i)
            for (int p = 0; p < k; ++p)
                A_row[i * k + p] = A[i * rsa + p * csa];
