...
inline void hgemm(
    ...
    if (alpha == 1.0f && beta == 0.0f && a_rowmajor_exact && b_rowmajor_exact && c_rowmajor_exact) {
        static thread_local AlignedBuffer a_f32_buf, b_f32_buf, c_f32_buf;
        a_f32_buf.ensure((size_t)m * (size_t)k);
        b_f32_buf.ensure((size_t)k * (size_t)n);
        float* A32 = a_f32_buf.data;
        float* B32 = b_f32_buf.data;
#if __F16C__
        for (int i = 0; i < m; ++i)
    ...
    static thread_local AlignedBuffer buf_a, buf_b, buf_c;
    ...
    if (static_cast<size_t>(m) > std::numeric_limits<size_t>::max() / static_cast<size_t>(n)) {
        throw std::overflow_error("hgemm: output size overflow");
    }
    buf_c.ensure(static_cast<size_t>(m) * static_cast<size_t>(n));
    float* C32 = buf_c.data;
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            C32[static_cast<size_t>(i) * n + j] =
                beta == 0.0f ? 0.0f : beta * static_cast<float>(C[i * rsc + j * csc]);
        }
    }
            ...
            for (int ii = 0; ii < m; ii += block_mc) {
                ...
                for (int ir = 0; ir < mc; ir += MR_ROW) {
                    ...
                    for (int jr = 0; jr < nc; jr += NR_ROW) {
                        ...
                        for (int i = 0; i < mr; ++i)
                            for (int j = 0; j < nr; ++j)
                                C32[static_cast<size_t>(ii + ir + i) * n + (jj + jr + j)] +=
                                    alpha * c_tmp[i * NR_ROW + j];
                    }
                }
            }
        }
    ...
    for (int i = 0; i < m; ++i)
        for (int j = 0; j < n; ++j)
            C[i * rsc + j * csc] = yt::float16(C32[static_cast<size_t>(i) * n + j]);
}
